...
contract('Auto', ([owner, investor]) => {
    ...
    before(async () => {
        ...
        auto = await Auto.new(jusdToken.address, mimicToken.address , farming.address , cjusdToken.address, swap.address)
    ...
    describe('cJUSD deployment', async () => {
        it('Check cJUSD Token prerequisite', async () => {
            ...
            assert.equal(name, 'Compound Jack USD Token', "cJUSD Contract name correct")
            ...
            assert.equal(supply.toString(), tokens('10000000'), "cJUSD Contract total supply correct")
        })
    })
    ...
    describe('Farming deployment', async () => {
        it('Check Farming deployment', async () => {
            ...
            assert.equal(name, 'Mimic Governance Token Farming', "Mimic Farm name correct")
        })
        it('Farming contract has mimic tokens', async () => {
            let balance = await mimicToken.balanceOf(farming.address)
            assert.equal(balance.toString(), tokens('10000000'), "Farming contract has mimic tokens")
        })
    })
    ...
    describe('Auto-Compound deployment', async () => {
        it('Check Auto-Compound deployment', async () => {
            ...
            assert.equal(name, 'Auto-Compound', "Auto Compound contract name correct")
        })
    ...
    describe('Auto-Compound Function', async () => {
        it('Check stake function', async () => {
            ...
            assert.equal(result.toString(), tokens('10000'), 'investor JUSD balance correct')
            //Deposit function
            await jusdToken.approve(auto.address, tokens('10000'), { from: investor })
            await auto.deposit(tokens('10000'),{from:investor})
            //check investor jusd balance
            result = await jusdToken.balanceOf(investor)
            assert.equal(result.toString(),tokens('0'), 'investor JUSD balance after stake correct')
            //check investor cjusd balance
            result = await cjusdToken.balanceOf(investor)
            assert.equal(result.toString(),tokens('10000'),'investor cJUSD balance correct')
            //check cJUSD auto-compound balance
            result = await cjusdToken.balanceOf(auto.address)
            assert.equal(result.toString(),tokens('9990000'),'cJUSD autocompound correct')
            //check auto-compound farm staking balance
            result = await farming.stakingBalance(auto.address)
            assert.equal(result.toString(),tokens('10000'),'auto-compound farming balance correct')
            //check auto-compound wallet
            result = await jusdToken.balanceOf(auto.address)
            assert.equal(result.toString(),tokens('0'),'auto-compound wallet balance correct')
